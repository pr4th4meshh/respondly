...
export async function PUT(
  ...
  try {
    ...
    //   const oldField = form.fields.find((f: { _id: { toString: () => any } }) => f._id.toString() === newField._id.toString());
    ...
    // Ensure to type newField as Field
    fields.forEach((newField: Field) => {
      const oldField = form.fields.find(
        (f: Field) => f._id.toString() === newField._id.toString()
      )
      if (oldField && oldField.label !== newField.label) {
        labelMap[oldField.label] = newField.label // This should work without errors now
      }
    })
    ...
    if (form.responses && form.responses.length > 0) {
      form.responses = form.responses.map((response: { label: string }) => {
        if (labelMap[response.label]) {
          console.log(
            `Updating response label from "${response.label}" to "${
              labelMap[response.label]
            }"`
          )
    ...
    console.error(
      "Error updating form and responses:",
      error.message,
      error.stack
    )
    return NextResponse.json(
      { success: false, error: "Failed to update form and responses" },
      { status: 500 }
    )
  }
}
